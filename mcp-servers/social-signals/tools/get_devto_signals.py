"""get_devto_signals — MCP tool handler.

Fetches top Dev.to articles for the requested tech stacks.
Dev.to is always available (public API, no key required).

JSON-RPC method: ``get_devto_signals``
Params: ``GetDevToSignalsParams`` (see models.py)
...
from models import GetDevToSignalsParams, SocialSignalsResult
...
_TOOL_NAME = "get_devto_signals"


async def handle_get_devto_signals(
    params: dict[str, Any],
    request: Request,
    ...
    *,
    ...
    with _tracer.start_as_current_span("tool.get_devto_signals") as span:
        ...
        try:
            try:
                p = GetDevToSignalsParams(**params)
            except ValidationError as exc:
                raise JsonRpcError(
                    JsonRpcErrorCode.INVALID_PARAMS,
                    "Invalid get_devto_signals parameters",
                    data=exc.errors(),
                )
            ...
            devto_client = clients.get("devto")
            if not devto_client:
                raise JsonRpcError(JsonRpcErrorCode.UPSTREAM_ERROR, "Dev.to client not configured")

            signals = await devto_client.search(
                p.stacks,
                p.limit,
            ...
            logger.error(
                "get_devto_signals.unhandled_error",
                error=str(exc),
                correlation_id=correlation_id,
                exc_info=True,
            )
