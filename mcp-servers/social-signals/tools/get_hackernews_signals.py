"""get_hackernews_signals — MCP tool handler.

Fetches top HackerNews stories, Ask HN, and Show HN posts matching
the requested tech stacks via the Algolia HN Search API.

JSON-RPC method: ``get_hackernews_signals``
Params: ``GetHackerNewsSignalsParams`` (see models.py)
...
from models import GetHackerNewsSignalsParams, SocialSignalsResult
...
_TOOL_NAME = "get_hackernews_signals"


async def handle_get_hackernews_signals(
    params: dict[str, Any],
    request: Request,
    ...
    *,
    ...
    with _tracer.start_as_current_span("tool.get_hackernews_signals") as span:
        ...
        try:
            try:
                p = GetHackerNewsSignalsParams(**params)
            except ValidationError as exc:
                raise JsonRpcError(
                    JsonRpcErrorCode.INVALID_PARAMS,
                    "Invalid get_hackernews_signals parameters",
                    data=exc.errors(),
                )
            ...
            hn_client = clients.get("hackernews")
            if not hn_client:
                raise JsonRpcError(JsonRpcErrorCode.UPSTREAM_ERROR, "HackerNews client not configured")

            signals = await hn_client.search(
                p.stacks,
                p.limit,
                correlation_id=correlation_id,
                tags=p.tags or ["story"],
            )
            ...
            logger.info(
                "get_hackernews_signals.completed",
                stacks=p.stacks,
                count=len(signals),
                user_id=user_id,
                correlation_id=correlation_id,
            ...
            logger.error(
                "get_hackernews_signals.unhandled_error",
                error=str(exc),
                correlation_id=correlation_id,
                exc_info=True,
            )
